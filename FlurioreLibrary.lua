...
function FlurioreLib:MakeNotify(NotifyConfig)
	...
	NotifyConfig.Title = NotifyConfig.Title or "Gravity Hub"
...
function FlurioreLib:MakeGui(GuiConfig)
	...
	GuiConfig.NameHub = GuiConfig.NameHub or "Gravity Hub"
	GuiConfig.Description = GuiConfig.Description or "by realtboy"
	...
	DropShadowHolder.ZIndex = 0
	DropShadowHolder.Name = "DropShadowHolder"
	DropShadowHolder.Parent = GravityGui
	
  DropShadowHolder.Position = UDim2.new(0, (GravityGui.AbsoluteSize.X // 2 - DropShadowHolder.Size.X.Offset // 2), 0, (GravityGui.AbsoluteSize.Y // 2 - DropShadowHolder.Size.Y.Offset // 2))
	DropShadow.Image = "rbxassetid://6015897843"
	...
	function GuiFunc:DestroyGui()
		if CoreGui:FindFirstChild("GravityGui") then 
			GravityGui:Destroy()
		end
	end
    ...
    function SliderFunc:Set(Value)
        ...
        local pct = (Value - SliderConfig.Min) / (SliderConfig.Max - SliderConfig.Min)
        SliderDraggable.Size = UDim2.new(pct, 0, 0, 1)
    end
    ...
    local function updateSlider(input)
        ...
        if frameSize <= 0 then return end
        local mouseX = input and input.Position.X or UserInputService:GetMouseLocation().X
        local SizeScale = math.clamp((mouseX - framePos) / frameSize, 0, 1)
